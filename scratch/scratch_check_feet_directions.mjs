...
async function analyzeFeet(filePath) {
  ...
  for (const node of doc.getRoot().listNodes()) {
    ...
    if (name.includes('leftfoot') || name === 'mixamorig:leftfoot') foot = node;
    if (name.includes('lefttoebase') || name === 'mixamorig:lefttoebase') toe = node;
  }
  ...
  if (foot && toe) {
    ...
    console.log(`  LeftFoot world pos: [${pFoot.map(v=>v.toFixed(4)).join(', ')}]`);
    console.log(`  LeftToeBase world pos: [${pToe.map(v=>v.toFixed(4)).join(', ')}]`);
    console.log(`  Direction vector (Foot -> Toe): [${dir.map(v=>v.toFixed(4)).join(', ')}]`);
  } else {
    console.log('  Could not find LeftFoot or LeftToeBase.');
  }
}

async function main() {
  await analyzeFeet('d:\\DEV\\BJS Character Controller V2\\assets\\characters_test\\gang.glb');
  await analyzeFeet('d:\\DEV\\BJS Character Controller V2\\assets\\character_animated.glb');
}
