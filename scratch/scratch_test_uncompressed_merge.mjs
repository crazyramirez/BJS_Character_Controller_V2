...
async function main() {
  ...
  console.log('Extensions used:', charDoc.getRoot().listExtensionsUsed().map(ext => ext.extensionName));

  // Dispose Draco extension to force uncompressed output
      ...
      for (let i = 0; i < aOrigJ.length; i++) {
        ...
        if (weight > 0.001) {
          ...
          if (nameOrig !== nameMerged) {
            mismatches++;
            if (mismatches <= 5) {
              console.log(`    Mismatch in mesh "${name}" prim [${pIdx}] vertex element ${i}: origJointIndex=${idxOrig} ("${boneOrigNode?.getName()}") <-> mergedJointIndex=${idxMerged} ("${boneMergedNode?.getName()}")`);
            }
          }
        }
      }

      console.log(`Mesh "${name}" prim [${pIdx}]: total checked = ${totalChecked}, mismatches = ${mismatches}`);
    }
  }
}
